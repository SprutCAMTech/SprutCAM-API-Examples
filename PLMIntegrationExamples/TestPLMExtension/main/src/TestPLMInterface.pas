...
uses
  System.SysUtils, IOUtils, IDL.SprutCAMTech.STPLMInterface, TestPLMParameters, TestPLMItems;
...
type
  ...
  TTestPLMInterface = class(TInterfacedObject, IPLMInterface)
  public
    constructor Create;
    procedure SetLanguage(LanguageID: LongWord; LngCodePage: Byte); safecall;

    // Вызывается при открытии формы ASetup,
    // если подключено ПЛМ расширение.
    ...
    // Отображение элементов в дереве ПЛМ на PLMImportForm
...
const
  ...
  PLMExtensionTempDir = '..\TestPLMExtension\TestPLMExtensionData';
...
begin
  for i := 0 to Pred(Items.Count) do
    case Items[i].Type_ of
      // Для модели и заготовки используется один и тот же файл,
      // т.к. они имеют одинаковый формат и похожую механику.
      ...
      itProject: ; // Для этого типа не требуются дополнительные файлы.
